...
// - 객체지향의 3속성
// -- 은닉성 (접근지정자)
// -- 상속성 (is a 관계가 한쪽 방향으로만 성립) ... Cat is a Animal
// -- 다형성 (함수 오버로딩, 함수 오버라이딩 ... (가상함수))

// - 빠른 초기화 영역... 느린 초기화 영역
// - 연산자 오버로딩
// - 멤버 함수 연습용 실습
// -- 멤버함수는 따로 독립적으로 호출(-> 객체가 호출해줘야 함)...멤버함수 호출은 무조건 객체가 필요하다.
...
// Stack (지역변수, 매개변수, 반환구조값)
// -> 생성주기 : 함수가 호출되서 시작할 때 생성 / 함수가 끝났을 때 해제
// -> 단점 : 메모리를 제어할 수 없다.
// 
// 
// Heap : 사용자 영역(프로그래머 영역)
